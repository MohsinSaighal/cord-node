...
export const useSupabaseTasks = (user: UserData, onUserUpdate: (user: UserData) => void) => {
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      // Load tasks directly from database with manual approach for better reliability
      console.log('Loading tasks manually from database...');
      ...
      if (!allTasks || allTasks.length === 0) {
        console.log('⚠️ No tasks returned from database');
        setLoadError('No tasks found in the database');
        setTasks([]);
        setLoading(false);
        return;
      }

      console.log('✅ Loaded tasks from database:', allTasks.length);
      ...
      if (userTasksError) {
        console.error('❌ Error loading user tasks:', userTasksError);
        setLoadError('Failed to load user task progress: ' + userTasksError.message);
        // Continue with empty user tasks rather than failing
      }

      const userTasksData = userTasks || [];
      console.log('✅ Loaded user tasks from database:', userTasksData.length);

      // Create a Map of user task data for fast lookup
      ...
      const tasksWithProgress = allTasks.map(task => {
        ...
        // Check if task is completed in database
        const isCompletedInDB = userTask?.completed || false;
        const userTaskProgress = userTask?.progress || 0;
        
        let progress = 0;
        let completed = isCompletedInDB;

        // For completed tasks, always trust the database
        if (completed) {
          progress = task.max_progress;completeTask
        } else if (task.id === 'social-media-master') {
          // For social-media-master, count completed social tasks
          const socialTasksCompleted = allTasks
            .filter(t => t.type === 'social')
            .map(t => t.id)
            .filter(id => userTasksMap.has(id) && userTasksMap.get(id).completed)
            .length;
          
          progress = socialTasksCompleted;
        } else {
          // Calculate dynamic progress for non-completed tasks
          progress = calculateTaskProgress(task.id, userTaskProgress, user);
        }
        ...
        return {
          ...
          socialUrl: task.social_url || undefined,
          expiresAt: task.expires_at ? new Date(task.expires_at) : undefined,
          claimedAt: userTask?.claimed_at ? new Date(userTask.claimed_at) : undefined
        };
      });

      console.log('✅ Processed tasks with progress:', tasksWithProgress.length);
  ...
  const completeTask = useCallback(async (taskId: string) => {
    ...
    try {
      ...
      // Use the database function to safely complete the task
      console.log('🔄 Calling database function complete_user_task...');
      const { data: result, error: functionError } = await supabase
        .rpc('complete_user_task_with_referral', {
          p_user_id: user.id,
          p_task_id: taskId,
          p_reward_amount: baseReward
        });
      ...
      console.log('✅ Task completed successfully via database function:', result);

      // Update local user state with the new balance from database
      const updatedUser = {
        ...user,
        currentBalance: result.task_completion?.new_balance || result.new_balance,
      ...
      if (taskId === 'daily-checkin') {
        ...
        // Update user in database for daily check-in specific fields
        try {
          await updateUserInDatabase(updatedUser);
        } catch (updateError) {
          console.error('❌ Error updating user for daily check-in:', updateError);
      ...
      let notificationMessage = `+${Math.floor(result.reward || result.task_completion?.reward || 0)} CORD earned from "${task.title}"!`;
      
      // Add referral bonus info if applicable
      const referralAmount = result.referral_amount || 
                            (result.referral_reward && result.referral_reward.referral_reward) || 
                            0;
                            
      if (referralAmount > 0) {
        notificationMessage += ` (Referrer earned +${Number(referralAmount).toFixed(2)} CORD bonus!)`;
      }
