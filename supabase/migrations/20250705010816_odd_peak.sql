...
DECLARE
  ...
  user_mining_time integer;
  user_tasks_completed integer;
BEGIN
  -- Get current active epoch
  SELECT * FROM epochs WHERE is_active = true ORDER BY epoch_number DESC LIMIT 1 INTO current_epoch_record;
  ...
  -- Get total participants in this epoch
  SELECT COUNT(*) FROM user_epoch_progress 
  WHERE epoch_id = current_epoch_record.id
  INTO total_participants;

  -- Calculate mining time for this epoch (from mining sessions)
  SELECT COALESCE(SUM(
    CASE 
  ...
  AND start_time >= current_epoch_record.start_date
  INTO user_mining_time;

  -- Calculate tasks completed in this epoch
  SELECT COUNT(*)::integer
  FROM user_tasks ut
  JOIN tasks t ON ut.task_id = t.id
  WHERE ut.user_id = p_user_id 
  AND ut.completed = true
  AND ut.claimed_at >= current_epoch_record.start_date
  INTO user_tasks_completed;

  -- Update the user_epoch_progress with calculated values
  UPDATE user_epoch_progress 
  SET 
    epoch_mining_time = user_mining_time,
    epoch_tasks_completed = user_tasks_completed,
    updated_at = now()
  WHERE user_id = p_user_id AND epoch_id = current_epoch_record.id;
  ...
  RETURN jsonb_build_object(
    ...
    'epoch_description', current_epoch_record.description,
    'start_date', current_epoch_record.start_date,
    'end_date', current_epoch_record.end_date,
    'days_remaining', GREATEST(0, EXTRACT(days FROM current_epoch_record.end_date - now())::integer),
    'progress_percentage', LEAST(100, GREATEST(0, 
      EXTRACT(epoch FROM now() - current_epoch_record.start_date) / 
      EXTRACT(epoch FROM current_epoch_record.end_date - current_epoch_record.start_date) * 100
    ))::numeric,
    'user_earnings', COALESCE(user_progress.epoch_earnings, 0),
    'user_tasks_completed', user_tasks_completed,
    'user_mining_time', user_mining_time,
    'user_rank', user_rank,
    'total_participants', total_participants,
    'rewards_multiplier', current_epoch_record.rewards_multiplier,
