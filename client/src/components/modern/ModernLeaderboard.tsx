...
const LeaderboardEntry: React.FC<{
  ...
  const getRankDisplay = (rank: number) => {
    switch (rank) {
      ...
      case 3:
        return (
          <div className="flex items-center justify-center w-12 h-12 bg-gradient-to-br from-amber-500 to-amber-700 rounded-full shadow-lg">
            <Award className="w-6 h-6 text-white" />
          </div>
        );
      default:
        return (
          <div className="flex items-center justify-center w-12 h-12 bg-slate-700/50 rounded-full border border-slate-600/50">
            <span className="text-slate-300 font-bold text-lg">#{rank}</span>
          </div>
        );
    }
  };
  ...
  const getRankGradient = (rank: number) => {
    switch (rank) {
      ...
      default: return 'from-slate-700/20 to-slate-800/20 border-slate-700/30';
    }
  };

  return (
    <AnimatedCard className={cn(
      "p-6 transition-all duration-300 border backdrop-blur-sm",
      `bg-gradient-to-r ${getRankGradient(rank)}`,
      isCurrentUser 
        ? "ring-2 ring-cyan-500/50 bg-gradient-to-r from-cyan-500/10 to-purple-600/10" 
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <div className="flex items-center space-x-4 text-sm text-slate-400">
                <span>Account: {user.accountAge} years</span>
                <span>•</span>
        ...
        <div className="text-right">
          <div className="text-2xl font-bold">
            <GradientText className={cn(
              ...
              {user.totalEarned?.toLocaleString() || '0'} CORD
            </GradientText>
          </div>
...
export const ModernLeaderboard: React.FC<ModernLeaderboardProps> = ({ currentUser }) => {
  ...
  const totalEarned = leaderboard.reduce((sum, user) => sum + (user.totalEarned || 0), 0);
  const currentUserRank = currentUser ? leaderboard.findIndex(u => u.username === currentUser.username) + 1 : null;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-900/20 to-cyan-900/20">
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-1/5 left-1/6 w-80 h-80 bg-cyan-500/10 rounded-full blur-3xl animate-float" />
        <div className="absolute top-2/3 right-1/5 w-96 h-96 bg-blue-500/10 rounded-full blur-3xl animate-float" style={{ animationDelay: '3s' }} />
        <div className="absolute top-1/2 left-3/4 w-72 h-72 bg-indigo-500/10 rounded-full blur-3xl animate-float" style={{ animationDelay: '6s' }} />
      </div>
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12 animate-slide-up">
          ...
          <p className="text-xl text-slate-300 max-w-2xl mx-auto">
            Compete with miners worldwide and climb the rankings to earn recognition and rewards
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
          ...
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:border-cyan-500/50 focus:bg-slate-800/70 transition-all duration-300"
            />
          </div>
        </div>
        ...
        {filteredLeaderboard.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-bold text-slate-300 mb-2">No miners found</h3>
            <p className="text-slate-500">Try adjusting your search or time period</p>
          </div>
        )}
      </div>
    </div>
