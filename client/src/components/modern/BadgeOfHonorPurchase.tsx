...
export const BadgeOfHonorPurchase: React.FC<BadgeOfHonorPurchaseProps> = ({ 
  ...
  const showNotification = (title: string, description: string, variant?: 'destructive') => {
    ...
    try {
      if (typeof window !== 'undefined' && 'solana' in window) {
        ...
        if (solanaWallet?.isPhantom) {
          ...
          showNotification("Wallet Connected", "Successfully connected to Phantom wallet");
        } else {
      ...
      showNotification("Connection Failed", error.message || "Failed to connect wallet", "destructive");
    }
  };
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      const mockTransactionHash = `badge_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      
      // Simulate transaction processing delay
      await new Promise(resolve => setTimeout(resolve, 3000));

      // Call the badge purchase API
      const response = await fetch('/api/badge-purchases', {
        method: 'POST',
        headers: {
      ...
      await fetch(`/api/badge-purchases/${mockTransactionHash}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      showNotification("Purchase Successful!", "Badge of Honor has been added to your account");

    } catch (error: any) {
      setTransactionStatus('error');
      showNotification("Purchase Failed", error.message || "Failed to complete badge purchase", "destructive");
    } finally {
      setIsPaying(false);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-32 h-32 opacity-10">
        <Crown className="w-full h-full" style={{ color: 'var(--reward-gold)' }} />
      </div>
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            >
              <Award className="w-8 h-8" style={{ color: 'var(--reward-gold)' }} />
            </div>
        ...
        <div className="mb-6 space-y-3">
          {[
            ...
            { icon: <Crown className="w-4 h-4" />, text: "Exclusive Badge Display" }
          ].map((benefit, index) => (
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="text-2xl font-bold mb-1" style={{ color: 'var(--reward-gold)' }}>
              {BADGE_PRICE_SOL} SOL
            </div>
          ...
          <SimpleButton
            ...
            style={{
              background: transactionStatus === 'idle' 
                ? 'linear-gradient(135deg, var(--reward-gold) 0%, #ff9800 100%)'
                : undefined
            }}
          >
            <div className="flex items-center space-x-2">
              {transactionStatus === 'processing' && <Loader className="w-5 h-5 animate-spin" />}
              {transactionStatus === 'success' && <CheckCircle className="w-5 h-5" />}
              {transactionStatus === 'error' && <AlertCircle className="w-5 h-5" />}
              {transactionStatus === 'idle' && !isConnectedToWallet && <Wallet className="w-5 h-5" />}
              {transactionStatus === 'idle' && isConnectedToWallet && <Award className="w-5 h-5" />}
              
              <span>
                {transactionStatus === 'processing' && 'Processing...'}
                {transactionStatus === 'success' && 'Success!'}
                {transactionStatus === 'error' && 'Try Again'}
                {transactionStatus === 'idle' && !isConnectedToWallet && 'Connect Wallet'}
                {transactionStatus === 'idle' && isConnectedToWallet && 'Purchase Badge'}
              </span>
            </div>
          </SimpleButton>
        </div>
        ...
        {isConnectedToWallet && (
          <div className="mt-4 p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
            <div className="flex items-center space-x-2">
              <CheckCircle className="w-4 h-4" style={{ color: 'var(--brand-secondary)' }} />
              <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>
