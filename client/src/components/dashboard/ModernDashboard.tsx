...
import { Coins, TrendingUp, Users, Clock, Award, Zap, Star, Target } from "lucide-react"
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        "bg-gradient-to-br from-gray-800/70 to-gray-900/70", // Slightly darker base for contrast
        "border border-gray-700/60 backdrop-blur-sm", // Adjusted border
        "hover:border-gray-600/60 transition-all duration-300",
      )}
    >
      {/* Background glow effect */}
      <div className={cn("absolute inset-0 opacity-50 bg-gradient-to-br", gradient)} />{" "}
...
const QuickActions: React.FC<{ user: any }> = ({ user }) => {
  ...
  return (
    <AnimatedCard
      className="p-6 bg-gradient-to-br from-gray-800/60 to-gray-900/60 border-gray-700/60" // Adjusted background and border
      delay={0.3}
    >
      <h3 className="text-lg font-bold text-white mb-4 flex items-center">
        <Target className="w-5 h-5 mr-2 text-blue-400" /> Quick Stats
      </h3>
      <div className="grid grid-cols-2 gap-4">
        {actions.map((action, index) => (
          <motion.div
            ...
            className="flex items-center justify-between p-3 rounded-lg bg-gray-700/40 border border-gray-600/40" // Adjusted inner item background and border
          >
            <div className="flex items-center space-x-2">
              <div className={action.color}>{action.icon}</div>
              <span className="text-sm text-gray-300">{action.label}</span>
            </div>
            <span className={cn("text-sm font-bold", action.color)}>{action.value}</span>
          </motion.div>
        ))}
      </div>
    </AnimatedCard>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950">
      ...
      <div className="fixed inset-0 pointer-events-none">
        ...
        />
        <div
          className="absolute top-1/2 left-3/4 w-48 h-48 bg-rose-500/10 rounded-full blur-3xl animate-float"
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          ...
          <StatCard
            ...
            gradient="from-rose-400 to-pink-600" // New vibrant gradient
            delay={2}
          />
          <StatCard
            title="Active Miners"
            ...
            gradient="from-red-400 to-orange-500" // New vibrant gradient
            delay={3}
            trend={5}
          />
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="xl:col-span-2 space-y-8">
            ...
            <ModernTaskPanel tasks={tasks} onCompleteTask={onCompleteTask} />
          </div>
          ...
          <div className="space-y-8">
            ...
            >
              <h3 className="text-lg font-bold text-white mb-4 flex items-center">
                <Clock className="w-5 h-5 mr-2 text-green-400" /> Recent Activity
              </h3>
              <div className="space-y-3">
                {[
                  ...
                  <motion.div
                    ...
                    className="flex items-center justify-between p-3 rounded-lg bg-gray-700/40" // Adjusted inner item background
                  >
                    <span className="text-sm text-gray-300">{activity.action}</span>
                    <span className={cn("text-xs font-medium", activity.color)}>{activity.time}</span>
                  </motion.div>
                ))}
              </div>
            </AnimatedCard>
